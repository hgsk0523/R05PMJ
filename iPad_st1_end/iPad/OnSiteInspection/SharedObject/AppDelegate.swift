...
class AppDelegate: UIResponder {
    ...
    // HTアプリのバンドルID
    private let htBundleID: String? = nil
...
extension AppDelegate: UIApplicationDelegate {
    ...
    func application(_ application : UIApplication, open url: URL, options : [UIApplication.OpenURLOptionsKey: Any] = [:]) -> Bool {
        ...
        log.info("バンドルID「\(sourceApplication ?? "nil")」による起動")
        log.info("customURLscheme「\(url)」")
        // HTアプリからの呼び出しのみ許可
        if (sourceApplication == htBundleID) {
            do {
                // 保存先にフォルダがなければ作成
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        let sourceApplication: String? = launchOptions?[UIApplication.LaunchOptionsKey.sourceApplication] as? String
        // HTアプリ又はホーム上からの呼び出しの場合のみ許可
        if (sourceApplication == nil || sourceApplication == "") {
            // アプリケーション連携時に全てのキャッシュを削除
            URLCache.shared.removeAllCachedResponses()
            // ストレージとメモリ上からキャッシュ保存容量を0に設定
