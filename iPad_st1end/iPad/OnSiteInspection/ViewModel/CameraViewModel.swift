...
protocol CameraViewModelProtocol {
    ...
    // 撮影例画像の保存
    func saveExplanationImage(handler: @escaping () -> Void) throws
    /// カメラの権限確認
    ...
    func saveExplanationImage(handler: @escaping () -> Void) throws {
        do {
            ...
            samplePhotoData.forEach { photodata in
                let path = photodata.s3_image_path + photodata.filename_name
                var uiImage: UIImage? = nil
                // 撮影例画像をS3から取得
                dispatchQueue.async(group: dispatchGroup) {
                    [weak self] in
                    dispatchGroup.enter()
                    ApiService.shared.getS3ImageData(bucketName: S3_BUCKET_NAME, s3Path: path) { image in
                        uiImage = image
