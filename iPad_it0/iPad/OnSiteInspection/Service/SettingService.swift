...
class SettingService: NSObject {
    ...
    // 撮影例画像のローカルパス
    var exampleImageLocalPath: String?
            ...
            setting.photoSample.forEach { photoSampleData in
                ...
                photoSampleData.photoInfo?.forEach { photoInfoData in
                    ...
                    ApiService.shared.getS3ImageData(bucketName: S3_BUCKET_NAME, s3Path: "\(photoInfoData.s3Path)\(photoInfoData.fileName)") { image in
                        ...
                        // 撮影例マスタを作成する
                        guard let exampleImageData = try? self.createExampleImageData(photoData: photoInfoData, photoSampleData: photoSampleData, setting: setting) else { return }
                        // 新しい撮影例マスタを保存する
    ...
    func saveExampleImageToDevice(image: UIImage?, fileName: String, filePath: String) throws {
        ...
        do {
            ...
            // ローカルパスを保存
            self.exampleImageLocalPath = filePath
            // 撮影例画像をデータ型に変換
    ...
    /// - Returns: 撮影例データ
    func createExampleImageData(photoData: PhotoInfo, photoSampleData: PhotoSample, setting: Settings) throws -> TBL_M_SAMPLE_PHOTO {
        do {
            guard let exampleImageLocalPath = self.exampleImageLocalPath else {
                throw ErrorID.ID9999
            }
            /// 撮影例データを作成
            ...
            /// ローカルパス
            exampleImageData.local_image_path = exampleImageLocalPath
            /// S3パス
    ...
    func setSettingFile(localJsonPath: URL, currentSettingJson: Data?, newSettingJson: Data) throws {
        do {
            ...
            if let currentSettingJson = currentSettingJson {
                ...
                // サーバーと今ローカルにある設定情報に差分があるかどうかを判別
                if (currentSettingInfo.version >= newSettingInfo.version) {
                    // 端末の設定ファイル情報をinfoに保存
